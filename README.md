...
A [Streamlit](https://streamlit.io) app ([`app.py`](app.py)) serves the WDBC model interactively.
...
- **30 features is an unusable form.** The app shows the six highest-importance features as sliders,
  defaults the other 24 to the dataset median behind an expander, and offers one-click example cases
  loaded from real dataset rows — including *the hard case* that the tree models misclassify.
...
the feature ranges and the example rows into a single **20 KB** `model_bundle.npz`, and
...
It opens at `http://localhost:8501`. If you retrain the model, regenerate the bundle first:
...
- **Multi-page app** — add a batch CSV upload and an "about the model" page with the ROC curve,
  confusion matrix and cross-validation charts from the notebook
